...
void timers_init(timers_state_t * timers_state)
{
    ...
    timers_setup_timer(TIMER_COUNTER0, TIMER_MODE_CTC, MS_TO_uS(1));
    TIMSK0 |= (1 << OCIE0A); // Unmask interrupt for output compare match A on TC0

    //--------------------------- YELLOW ----------------------------------
    // Timer Interrupt Using TC3
    timers_setup_timer(TIMER_COUNTER3, TIMER_MODE_CTC, MS_TO_uS(g_timers_state->yellow_period));
    ...
    timers_setup_timer(TIMER_COUNTER1, TIMER_MODE_CTC, MS_TO_uS(g_timers_state->green_period));
